<template>
    <div>
      ...
      <template v-else>
        <table :class="['table', css.tableClass]"> <!-- no fixed header - regular table -->
          <thead>
            <tr>
                ...
                <template v-for="(field, fieldIndex) in tableFields">
                  <template v-if="field.visible">
                    <template v-if="isSpecialField(field.name)">
                      ...
                      <td v-if="extractName(field.name) == '__actions'"
                        :key="fieldIndex"
                      >
                        <template v-for="(action, index) in itemActions">
                            <button type="button" @click="clickActions(item, action, field.param, action)" :class="action.class" :key="index"><i :class="action.icon" style="font-size: 20px; padding: 0 auto"></i></button>
                        </template>
                      </td>
                      <td v-if="extractName(field.name) == '__is_checked'"
                        :key="fieldIndex"
                      > 
                        <span :class="showIsCheked(field.dataName, item)"></span>
                      </td>
                      <td v-if="extractName(field.name) == '__status'"
                        :key="fieldIndex"
                      > 
                        <span class="p-2 badge fs-20" :class="showStatusClass(field.dataName, item)">{{ showStatusText(field.dataName, item) }}</span>
                      </td>
                      <td v-if="extractName(field.name) == '__time_stamp'"
                        :key="fieldIndex"
                      > 
                        {{ convertTimestamp(field.dataName, item) }}
                      </td>
                      <td v-if="extractName(field.name) == '__price'"
                        :key="fieldIndex"
                      > 
                        {{ globalFormatNumber(item[field.dataName], ',') }} đ
                      </td>
                      <td v-if="extractName(field.name) == '__number'"
                        :key="fieldIndex"
                      > 
                        {{ globalFormatNumber(item[field.dataName], '.') }}
                      </td>
                      <td v-if="extractName(field.name) == '__list'"
                        :key="fieldIndex"
                      > 
                        {{ renderListData(item, field.listName, field.dataName) }}
                      </td>
                      <td v-if="extractName(field.name) == '__link'"
                        :key="fieldIndex"
                      > 
                        <div class="text-info p-cursor" @click="redirectLink(item, field)">{{ item[field.dataName] }}</div>
                      </td>
                    </template>
                    <template v-else>
                      ...
                      <td v-else 
                        :key="fieldIndex"
                        :class="field.dataClass"
                        v-html="renderItemData(item, field.name, field.defaultName)"
                        @click="onCellClicked(item, field, $event)"
                        @dblclick="onCellDoubleClicked(item, field, $event)"
                        @contextmenu="onCellRightClicked(item, field, $event)"
                      ></td>
        ...
        <Pagination :tablePagination="tablePagination" :paramsPage="currentPage" @changePage="changePage"/>  
      </template>
    </div>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      currentPage: this.initialPage && !isNaN(this.initialPage) ? this.initialPage : 1,
      selectedTo: [],
      visibleDetailRows: [],
  ...
  methods: {
    showStatusClass: function(dataName, item) {
      let name_index = dataName.split('.');
      let status = '';
      for(let i in name_index){
        let index = name_index[i];
        if(status){
          status = status[index];
        }else{
          status = item[index];
      ...
      if(status == 'active' || status == 'Active' || status == 'success' || status == 'show') {
        return 'badge-success';
      }else if(status == 'inactive' || status == 'Inactive' || status == 'cancel' || status == 'banned' || status == 'hide') {
        return 'badge-danger';
      }
    ...
    showStatusText: function(dataName, item) {
      let name_index = dataName.split('.');
      let status = '';
      for(let i in name_index){
        let index = name_index[i];
    ...
    convertTimestamp: function(dataName, item){
      let name_index = dataName.split('.');
      let timestamp = '';
      for(let i in name_index){
        let index = name_index[i];
      ...
      if(!timestamp){
        ...
        return this.globalConvertTimestamp(timestamp, 'DD/MM/YYYY HH:mm');
      }
    },
    renderListData: function(item, listName, dataName) {
    ...
    redirectLink: function(item, field) {
        let params = {};
        params[field.param] = item[field.param];
        this.$router.push({ name: field.routeName, params });
        delete this.$route.query;
    },
    showIsCheked: function(dataName, item) {
      let data = item[dataName];
      if(data == '1'){
        return 'mdi mdi-checkbox-marked-circle gl-checked-icon';
      }else{
        return '';
      }
    },
      ...
      this.fields.forEach(function(field, i) {
        if (typeof (field) === 'string') {
          ...
          obj = {
            ...
            param: field.param,
            dataName: (field.dataName === undefined) ? true : field.dataName,
            listName: (field.listName === undefined) ? true : field.listName,
            routeName: (field.routeName === undefined) ? true : field.routeName,
          }
        }
    ...
    loadSuccess (response) {
      ...
      if(response.data.data.data.length && (response.data.data.last_page < this.currentPage || this.currentPage < 1)){
        this.refresh(response.data.data.last_page);
      }
    ...
    getAllQueryParams () {
      ...
      if (typeof(this.queryParams) === 'function') {
        params = this.queryParams(this.sortOrder, this.currentPage)
    ...
    clickActions: function(item, action, path){
      const { callBack } = action;
      let id = this.getObjectValue(item, path, '');
      this.$emit(callBack, id);
    },
    renderItemData: function(item, name, defaultName){
      let name_index = name.split('.');
      let data = '';
      for(let i in name_index){
        let index = name_index[i];
        if(data){
      ...
      let default_data = '';
      if(defaultName){
        let default_index = defaultName.split('.');
        for(let i in default_index){
          let index = default_index[i];
          if(default_data){
            default_data = default_data[index];
          }else{
            default_data = item[index];
          }
        }
      }
      if (data !== '') {
