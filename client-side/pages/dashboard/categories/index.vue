...
export default {
    ...
    created() {
        this.$store.commit('setActiveSidebar', { isActive: 'categories', subIsActive: 'category-list' });
        this.initUrl();
    },
    methods: {
        editItem: function(id){
            this.$router.push({ name: 'dashboard-categories-update-id', params: { id: id }, query: { return: this.$route.query.page } });
        },
        deleteItem: function(id){
            this.$http.post('categories/delete/' + id)
            .then( response => {
